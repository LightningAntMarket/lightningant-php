 ...
 * @throws LAP\Exception
 * @return void
 */
function E($msg, $code=0) {
    throw new LAP\Exception($msg, $code);
}
 ...
 * @return void|array
 */
function trace($value='[LAP]',$label='',$level='DEBUG',$record=false) {
    return LAP\LAP::trace($value,$label,$level,$record);
}
...
function I($name,$default='',$filter=null,$datas=null) {
    ...
    is_array($data) && array_walk_recursive($data,'LAP_filter');
    return $data;
}
...
function import($class, $baseUrl = '', $ext=EXT) {
    ...
    if (empty($baseUrl)) {
        ...
        }elseif (in_array($class_strut[0],array('LAP','Org','Behavior','Com','Vendor')) || is_dir(LIB_PATH.$class_strut[0])) {
            // 系统类库包和第三方类库包
            $baseUrl = LIB_PATH;
        }else { // 加载其他模块的类库
            $baseUrl = APP_PATH;
...
function D($name='',$layer='') {
    if(empty($name)) return new LAP\Model;
    static $_model  =   array();
    ...
    if(class_exists($class)) {
        ...
        $model      =   class_exists($class)? new $class($name) : new LAP\Model($name);
    }else {
        LAP\Log::record('D方法实例化没找到模型类'.$class,LAP\Log::NOTICE);
        $model      =   new LAP\Model(basename($name));
    }
...
function B($name, $tag='',&$params=NULL) {
    ...
    return \LAP\Hook::exec($name,$tag,$params);
}
 ...
 * 自定义异常处理
 * @param string $msg 异常消息
 * @param string $type 异常类型 默认为LAP\Exception
 * @param integer $code 异常代码 默认为0
 * @return void
 */
function throw_exception($msg, $type='LAP\\Exception', $code=0) {
    LAP\Log::record('建议使用E方法替代throw_exception',LAP\Log::NOTICE);
    if (class_exists($type, false))
        throw new $type($msg, $code);
    else
        LAP\LAP::halt($msg);        // 异常类型不存在则输出错误信息字串
}
...
function S($name,$value='',$options=null) {
    ...
    if(is_array($options)){
        ...
        $cache      =   LAP\Cache::getInstance($type,$options);
    }elseif(is_array($name)) { // 缓存初始化
        $type       =   isset($name['type'])?$name['type']:'';
        $cache      =   LAP\Cache::getInstance($type,$name);
        return $cache;
    }elseif(empty($cache)) { // 自动初始化
        $cache      =   LAP\Cache::getInstance();
    }
    if(''=== $value){ // 获取缓存
...
function F($name, $value='', $path=DATA_PATH) {
    ...
    if ('' !== $value) {
        if (is_null($value)) {
            ...
            if(false !== strpos($name,'*')){
                ...
                unset($_cache[$name]);
                return LAP\Storage::unlink($filename,'F');
            }
        } else {
            LAP\Storage::put($filename,serialize($value),'F');
            // 缓存数据
            $_cache[$name]  =   $value;
            return null;
        }
    ...
    if (isset($_cache[$name]))
        return $_cache[$name];
    if (LAP\Storage::has($filename,'F')){
        $value      =   unserialize(LAP\Storage::read($filename,'F'));
        $_cache[$name]  =   $value;
    } else {
        $value          =   false;
    }
 ...
 * @return string
 */
function xml_encode($data, $root='LAP', $item='item', $attr='', $id='id', $encoding='utf-8') {
    if(is_array($attr)){
        $_attr = array();
...
function session($name='',$value='') {
    ...
    if(is_array($name)) { // session初始化 在session_start 之前调用
        ...
        if(C('SESSION_TYPE')) { // 读取session驱动
            $type   =   C('SESSION_TYPE');
            $class  =   strpos($type,'\\')? $type : 'LAP\\Session\\Driver\\'. ucwords(strtolower($type));
            $hander =   new $class();
...
function cookie($name='', $value='', $option=null) {
    ...
    if ('' === $value) {
        ...
        if (is_null($value)) {
            ...
            // 设置cookie
            if(is_array($value)){
                $value  = 'LAP:'.json_encode(array_map('urlencode',$value));
            }
