...
class CheckLangBehavior {
    ...
    private function checkLanguage() {
        ...
        if (C('LANG_AUTO_DETECT',null,true)){
            if(isset($_REQUEST[$varLang])){
                $langSet = $_REQUEST[$varLang];// url中设置了语言变量
                cookie('LAP_language',$langSet,3600);
            }elseif(cookie('LAP_language')){// 获取上次用户的选择
                $langSet = cookie('LAP_language');
            }elseif($langSet){ // 默认语言
                 cookie('LAP_language',$langSet,3600);
        ...
        // 读取框架语言包
        $file   =   LAP_PATH.'Lang/'.LANG_SET.'.php';
