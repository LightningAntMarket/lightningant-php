...
class Sqlsrv extends Driver{
    protected $selectSql  =     'SELECT T1.* FROM (SELECT LAPphp.*, ROW_NUMBER() OVER (%ORDER%) AS ROW_NUMBER FROM (SELECT %DISTINCT% %FIELD% FROM %TABLE%%JOIN%%WHERE%%GROUP%%HAVING% %UNION%) AS LAPphp) AS T1 %LIMIT%%COMMENT%';
    // PDO连接参数
