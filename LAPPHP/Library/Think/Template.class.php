...
class  Template {
    ...
    protected function compiler($tmplContent) {
        ...
        // 添加安全代码
        $tmplContent =  '<?php if (!defined(\'LAP_PATH\')) exit();?>'.$tmplContent;
        // 优化生成的php代码
    ...
    public function parseTagLib($tagLib,&$content,$hide=false) {
        ...
        if(strpos($tagLib,'\\')){
            ...
            $className  =   'LAP\\Template\TagLib\\'.ucwords($tagLib);
        }
        $tLib       =   \LAP\LAP::instance($className);
        $that       =   $this;
    ...
    public function parseVar($varStr){
        ...
        if(!empty($varStr)){
            ...
            if('LAP.' == substr($var,0,6)){
                // 所有以LAP.打头的以特殊变量对待 无需模板赋值就可以输出
                $name = $this->parseLAPVar($var);
