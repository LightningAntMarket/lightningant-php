...
class TagLib {
    ...
    public function __construct() {
        ...
        $this->tpl     = \LAP\LAP::instance('LAP\\Template');
    }
    ...
    public function parseCondition($condition) {
        ...
        if(false !== strpos($condition, '$LAP'))
            $condition      =   preg_replace_callback('/(\$LAP.*?)\s/is', array($this, 'parseLAPVar'), $condition);
        return $condition;
    }
    ...
    public function autoBuildVar($name) {
        if('LAP.' == substr($name,0,6)){
            // 特殊变量
            return $this->parseLAPVar($name);
        }elseif(strpos($name,'.')) {
            $vars = explode('.',$name);
    ...
    /**
     * 用于标签属性里面的特殊模板变量解析
     * 格式 以 LAP. 打头的变量属于特殊模板变量
     * @access public
     * @param string $varStr  变量字符串
     * @return string
     */
    public function parseLAPVar($varStr){
        ...
        if(count($vars)>=3){
            ...
            switch($vars[1]){
                ...
                case 'VERSION':  $parseStr = 'LAP_VERSION';break;
