...
define('APPKEY','5SgpbdVJXxAqEXKVf6vlx7');
define('APPID','9Z8rvkX4nH8sRE9DXntafA');
define('MASTERSECRET','R5pny5zfeV787lnX2O1Nf9');
...
function getPushData($PushInfo =array()){
    define('CID',$PushInfo['cid']);
    pushMessageToSingleBatch($PushInfo);
}
...
function pushMessageToSingleBatch($content)
{
...
//    $template = IGtNotyPopLoadTemplateDemo();
    //$template = IGtLinkTemplateDemo();
    //$template = IGtNotificationTemplateDemo();
    $template = IGtTransmissionTemplateDemo($content);
...
function IGtTransmissionTemplateDemo($pushdata){
        $template =  new IGtTransmissionTemplate();
        $template->set_appId(APPID);//应用appid
        $template->set_appkey(APPKEY);//应用appkey
        $template->set_transmissionType(2);//透传消息类型
        $template->set_transmissionContent(json_encode($pushdata));//透传内容
        //$template->set_duration(BEGINTIME,ENDTIME); //设置ANDROID客户端在此时间区间内展示消息
    //APN简单推送
//        $apn = new IGtAPNPayload();
//        $alertmsg=new SimpleAlertMsg();
...
//        $template->set_apnInfo($apn);
    ...
    //APN高级推送
        $apn = new IGtAPNPayload();
        $alertmsg=new DictionaryAlertMsg();
        $alertmsg->body=$pushdata['content'];
        $alertmsg->actionLocKey="ActionLockey";
        $alertmsg->locKey=$pushdata['content'];//"LocKey";
        $alertmsg->locArgs=array("locargs");
        $alertmsg->launchImage="launchimage";
//        IOS8.2 支持
        $alertmsg->title="白送";
        $alertmsg->titleLocKey="白送";
        $alertmsg->titleLocArgs=array("TitleLocArg");
    ...
    //PushApn老方式传参
//    $template = new IGtAPNTemplate();
//          $template->set_pushInfo("", 1, $pushdata['content'], "", "", "", "", "");
