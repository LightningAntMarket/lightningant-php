...
class huanxin
{
	 ...
	 * 环信-服务器端REST API
	 */
	private $client_id = 'YXA6UXNWYJabEeew_bsXJB-oGQ';
	private $client_secret = 'YXA6LlDZiNejvuRQwgSbc5vaaWeE01s';
	private $org_name = 'yibaisong';
	private $app_name = 'baisong-inter';
	private $url = 'https://a1.easemob.com/yibaisong/baisong-inter/';
	/**
	 * 开放注册模式
	 *
     ...
     * 修改群组信息
     */
    public function upGroups($group_id,$option){
        $url = $this->url . "chatgroups/" . $group_id;
        $access_token = $this->getToken ();
        $header [] = 'Authorization: Bearer ' . $access_token;
     ...
     * 获取一个用户参与的所有群组
     */
    public function usersGroups($hxid){
        $url = $this->url . "users/" . $hxid.'/joined_chatgroups';
        $access_token = $this->getToken ();
        $header [] = 'Authorization: Bearer ' . $access_token;
        $result = $this->postCurl ( $url,'',$header,$type = "GET");
        return $result;
     ...
     * 分页获取群组成员
     */
    public function groupsUserslist($group_id,$pagenum,$pagesize){
     ...
     * 设置群管理员
     */
    public function groupsAdmin($group_id,$option){
        $url = $this->url . "chatgroups/" . $group_id.'/admin';
        $access_token = $this->getToken ();
        $header [] = 'Authorization: Bearer ' . $access_token;
        $result = $this->postCurl ( $url,$option,$header,$type = "POST");
        return $result;
    }
     ...
     * 删除管理员
     */
    public function deleteAdmin($group_id,$hxid){
        $url = $this->url . "chatgroups/" . $group_id.'/admin/'.$hxid;
        $access_token = $this->getToken ();
        $header [] = 'Authorization: Bearer ' . $access_token;
        $result = $this->postCurl ( $url,'',$header,$type = "DELETE");
        return $result;
     ...
     * 群管理员列表
     */
    public function adminList($group_id){
        $url = $this->url . "chatgroups/" . $group_id.'/admin';
        $access_token = $this->getToken ();
        $header [] = 'Authorization: Bearer ' . $access_token;
        $result = $this->postCurl ( $url,'',$header,$type = "GET");
