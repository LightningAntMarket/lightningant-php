...
//------------------------------------------------------用户体系	
    ...
    public function __construct()
    {
        ...
        $options['client_id'] = 'YXA6pqlIUAH3EeWiHy31xCQ1cQ';
        $options['client_secret'] = 'YXA6Sza-q7OdruWnZLaJlhxNAknvxus';
        $options['org_name'] = 'yibaisong';
        $options['app_name'] = 'yibaisong907';
        $options['url'] = 'https://a1.easemob.com/';
    ...
    */
    ...
    /*
        发送文本消息
    */
    function sendText($from, $target_type = '', $target = '', $content = '', $ext = '')
    {
 ...
 * 加入黑名单
 * @param $myPhone 我的手机号
 * @param $blackPhone  要加入的手机号
 */
function addUserForBlacklist($myPhone, $blackPhone)
{
    $obj = new huanxin();
    $usernames = array(
        "usernames" => array("$myPhone", "$blackPhone")
    );

   $obj->addUserForBlacklist($myPhone, $usernames);
 ...
 *
 */
function deleteUserFromBlacklist($myPhone, $blackPhone)
{
    $obj = new huanxin();
    $obj->deleteUserFromBlacklist("$myPhone", "$blackPhone");
}


/**
 * @param $from   来自谁   string
 * @param $to    发给谁  string
 * @param $content  消息内容  string
 * @param $ext    扩展消息  array('gname'=>'自拍杆','gid'=>'1')
 * @param string $target_type    默认数值
 *
 * 环信 给用户发送消息。
 */

function sendText($from,$to,$content,$ext,$target_type = 'users'){

    $target = array($to);
    $obj = new huanxin();

    $obj->sendText($from ,$target_type , $target , $content, $ext );
}
